...
<table class="table table-striped">
    <thead>
        <tr>
            <th class="ps-0 pe-1" scope="col"><a href="{{route('admin_users', ['count' => $count, 'sorting' => 'id_'.$sort])}}"> Id</a>
                <form action="{{ route('admin_users') }}" method="GET">
                    <input type="text" class="form-control p-1" name="id_search" placeholder = "id">
                </form>
            </th>
            <th class="ps-1 pe-1"  scope="col"><a href="{{route('admin_users', ['count' => $count, 'sorting' => 'date_cr_'.$sort])}}">Дата созд.</a>
                <form action="{{ route('admin_users') }}" method="GET">
                    <input type="text" class="form-control p-1" name="date_cr_search" placeholder = "19-04-2025 день-месяц-год">
                </form>
            </th>
            <th class="ps-1 pe-1" scope="col"><a href="{{route('admin_users', ['count' => $count, 'sorting' => 'date_up_'.$sort])}}">Дата обн.</a>
                <form action="{{ route('admin_users') }}" method="GET">
            ...
            <th scope="col"><a href="{{route('admin_users', ['sorting' => 'activ_'.$sort])}}">ban</a></th>
